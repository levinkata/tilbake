...
namespace Tilbake.Application.Services
{
    public class MotorUseService : IMotorUseService
    {
        ...
        public async Task<int> AddAsync(MotorUseSaveResource resource)
        {
            ...
            await _unitOfWork.MotorUses.AddAsync(motorUse).ConfigureAwait(true);
        ...
        public async Task<int> DeleteAsync(MotorUseResource resource)
        {
            ...
            await _unitOfWork.MotorUses.DeleteAsync(motorUse).ConfigureAwait(true);
            return await Task.Run(() => _unitOfWork.SaveAsync()).ConfigureAwait(true);
        }

        public async Task<IEnumerable<MotorUseResource>> GetAllAsync()
        {
            var result = await Task.Run(() => _unitOfWork.MotorUses.GetAllAsync()).ConfigureAwait(true);
        ...
        {
            var result = await _unitOfWork.MotorUses.GetByIdAsync(id).ConfigureAwait(true);
        ...
        {
            var motorUse = _mapper.Map<MotorUseResource, MotorUse>(resource);
            await _unitOfWork.MotorUses.UpdateAsync(resource.Id, motorUse).ConfigureAwait(true);
