...
namespace Tilbake.Application.Services
{
    public class SalesTypeService : ISalesTypeService
    {
        ...
        public async Task<int> AddAsync(SalesTypeSaveResource resource)
        {
            ...
            await _unitOfWork.SalesTypes.AddAsync(salesType).ConfigureAwait(true);
        ...
        public async Task<int> DeleteAsync(SalesTypeResource resource)
        {
            ...
            await _unitOfWork.SalesTypes.DeleteAsync(salesType).ConfigureAwait(true);
            return await Task.Run(() => _unitOfWork.SaveAsync()).ConfigureAwait(true);
        }

        public async Task<IEnumerable<SalesTypeResource>> GetAllAsync()
        {
            var result = await Task.Run(() => _unitOfWork.SalesTypes.GetAllAsync()).ConfigureAwait(true);
        ...
        {
            var result = await _unitOfWork.SalesTypes.GetByIdAsync(id).ConfigureAwait(true);
        ...
        {
            var salesType = _mapper.Map<SalesTypeResource, SalesType>(resource);
            await _unitOfWork.SalesTypes.UpdateAsync(resource.Id, salesType).ConfigureAwait(true);
