            ...
            return await Task.Run(() => context.KlientRisks
                                                .Include(a => a.Risk)
                                                    .ThenInclude(r => r.AllRisk)
                                                        .ThenInclude(c => c.RiskItem)
                                                .Include(a => a.Risk)
                                                    .ThenInclude(r => r.Content)
                                                .Include(a => a.Risk)
                                                    .ThenInclude(r => r.House)
                                                        .ThenInclude(w => w.ResidenceType)
                                                .Include(a => a.Risk)
                                                    .ThenInclude(r => r.House)
                                                        .ThenInclude(w => w.RoofType)
                                                .Include(a => a.Risk)
                                                    .ThenInclude(r => r.House)
                                                        .ThenInclude(w => w.WallType)
                                                .Include(a => a.Risk)
                                                    .ThenInclude(r => r.Motor)
                                                        .ThenInclude(m => m.BodyType)
                                                .Include(a => a.Risk)
                                                    .ThenInclude(r => r.Motor)
                                                        .ThenInclude(m => m.DriverType)
                                                .Include(a => a.Risk)
                                                    .ThenInclude(r => r.Motor)
                                                        .ThenInclude(m => m.MotorModel)
                                                            .ThenInclude(m => m.MotorMake)
                                                .Include(a => a.Risk)
                                                    .ThenInclude(r => r.Motor)
                                                        .ThenInclude(m => m.MotorUse)
                                                .Include(a => a.Risk)
                                                    .ThenInclude(r => r.Motor)
                                                        .ThenInclude(m => m.MotorImprovements)
                                                .Include(a => a.Klient)
